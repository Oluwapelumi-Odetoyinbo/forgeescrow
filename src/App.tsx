...
import { Plus, Shield, Zap, Lock, CheckCircle2, AlertTriangle } from 'lucide-react';
...
const NetworkBanner = ({
  ...
  <div className="fixed top-0 left-0 right-0 z-[60] bg-warning/90 text-black py-2 px-4 flex items-center justify-center gap-3 text-sm font-medium">
    <AlertTriangle className="w-4 h-4" />
    <span>Wrong network detected. Please switch to {networkName}.</span>
    <button
      onClick={onSwitch}
      className="px-3 py-1 bg-black/20 rounded-full text-xs font-semibold hover:bg-black/30 transition-colors"
    >
      Switch Network
    </button>
  </div>
  ...
  const perks = [
    {
      ...
      title: 'Secure Escrow',
      desc: 'Military-grade smart contract protection for all your trade transactions.',
    },
    {
      icon: <Zap className="w-6 h-6" />,
      title: 'Instant Settlement',
      desc: 'No more waiting for banks. Get paid instantly in zkLTC upon delivery.',
    },
    {
      icon: <Lock className="w-6 h-6" />,
      title: 'Trustless Flow',
      desc: 'No intermediaries or geographic gatekeepers. Just pure code-driven trust.',
    },
  ];
  ...
  return (
    <div className="bg-background">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6" data-stagger>
            {steps.map((step, i) => (
              <div
                key={i}
                className="group relative bg-white/[0.03] border border-white/10 rounded-3xl p-8 hover:bg-white/[0.05] transition-colors overflow-hidden"
              >
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div data-reveal="left">
            ...
            <p className="text-zinc-400 mb-8 max-w-md">
              Leverage LitVM's programmable privacy to secure your trade finance. Only release funds
              when specific conditions are met.
            </p>
            <ul className="space-y-4 mb-8">
              {['Zk-proof settlement', 'Multi-signature milestones', 'Trustless arbitration'].map(
                (item, i) => (
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6" data-stagger>
            {perks.map((perk, i) => (
              <div
                key={i}
                className="p-8 rounded-3xl bg-zinc-900/50 border border-white/10 hover:-translate-y-2 hover:bg-zinc-900/70 transition-all duration-300"
              >
      ...
      <section className="py-24 px-6">
        <div
          className="max-w-5xl mx-auto relative rounded-[2.5rem] overflow-hidden border border-white/10 bg-zinc-900/50 p-12 md:p-20 text-center"
          data-reveal="scale"
        >
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-zinc-400 mb-10 max-w-lg text-sm md:text-base">
              Join the LiteForge Hackathon's premier RWA platform and experience seamless on-chain
              trade finance.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <Button className="px-8 py-4 text-base w-full sm:w-auto">Get Started Free</Button>
              <Button variant="secondary" className="px-8 py-4 text-base w-full sm:w-auto">
                Read Whitepaper
              </Button>
            </div>
          </div>
    ...
    <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10 mb-16">
      <div className="col-span-2 lg:col-span-2">
        ...
        <p className="text-sm text-zinc-500 max-w-xs mb-6">
          Bridging the gap between decentralized trade finance and the freelance economy. Secure,
          fast, and trustless.
        </p>
      ...
      <div>
        ...
        <ul className="space-y-3 text-sm text-zinc-500">
          <li>
            <a href="#" className="hover:text-[#c5f015] transition-colors">
              Escrows
            </a>
          </li>
          <li>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-background text-zinc-50 font-sans selection:bg-[#c5f015] selection:text-black">
      ...
      {isWrongNetwork && (
        <NetworkBanner networkName={expectedNetworkName} onSwitch={switchToLitVM} />
      )}
      ...
      <main
        className={clsx(
          !wallet.isConnected ? '' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-32',
          isWrongNetwork && 'mt-10'
        )}
      >
        {!wallet.isConnected ? (
          <LandingSections />
