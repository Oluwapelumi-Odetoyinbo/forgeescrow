...
export const EscrowDetails = ({ escrow, walletAddress, onBack, onUpdate }: EscrowDetailsProps) => {
  const { fundEscrow, completeMilestone, approveMilestone, raiseDispute, loading } = useContract();
  ...
  const getStatusColor = (status: EscrowStatus) => {
    switch (status) {
      case EscrowStatus.Created: return 'text-warning';
      case EscrowStatus.Funded:
      case EscrowStatus.InProgress: return 'text-secondary';
      case EscrowStatus.Completed: return 'text-success';
      case EscrowStatus.Cancelled:
      case EscrowStatus.Disputed: return 'text-error';
    ...
    >
      ...
      <div className="p-8 bg-gradient-to-br from-surface/80 to-surface/40 backdrop-blur-sm rounded-2xl border border-border">
        <div className="flex items-start justify-between mb-6">
          ...
          <div className={`px-4 py-2 rounded-full bg-surface border border-border ${getStatusColor(escrow.status)} font-semibold`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div>
            ...
            <p className="text-2xl font-bold text-primary">{parseFloat(escrow.totalAmount).toFixed(4)} zkLTC</p>
          </div>
          <div>
            <p className="text-sm text-textSecondary mb-1">Deposited</p>
            <p className="text-2xl font-bold text-text">{parseFloat(escrow.depositedAmount).toFixed(4)} zkLTC</p>
          </div>
          <div>
            <p className="text-sm text-textSecondary mb-1">Released</p>
            <p className="text-2xl font-bold text-success">{parseFloat(escrow.releasedAmount).toFixed(4)} zkLTC</p>
          </div>
          <div>
            <p className="text-sm text-textSecondary mb-1">Progress</p>
            <p className="text-2xl font-bold text-text">{completedMilestones}/{escrow.milestones.length}</p>
          </div>
        </div>
        ...
        <div className="p-6 bg-surface/50 backdrop-blur-sm rounded-xl border border-border">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="font-mono text-sm text-text">{escrow.client.slice(0, 10)}...{escrow.client.slice(-8)}</p>
            </div>
          </div>
          {isClient && (
        ...
        <div className="p-6 bg-surface/50 backdrop-blur-sm rounded-xl border border-border">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="font-mono text-sm text-text">{escrow.freelancer.slice(0, 10)}...{escrow.freelancer.slice(-8)}</p>
            </div>
          </div>
          {isFreelancer && (
      ...
      {isClient && escrow.status === EscrowStatus.Created && (
        <div className="p-6 bg-warning/10 border border-warning/20 rounded-xl">
          ...
          <p className="text-sm text-textSecondary mb-4">
            Remaining to fund: {(parseFloat(escrow.totalAmount) - parseFloat(escrow.depositedAmount)).toFixed(4)} zkLTC
          </p>
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right">
                <p className="text-2xl font-bold text-primary">{parseFloat(milestone.amount).toFixed(4)} zkLTC</p>
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              {isFreelancer && !milestone.completed && !milestone.approved && escrow.status === EscrowStatus.InProgress && (
                <button
      ...
      {/* Dispute Section */}
      {(escrow.status === EscrowStatus.Funded || escrow.status === EscrowStatus.InProgress) && !escrow.disputed && (
        <div className="p-6 bg-error/10 border border-error/20 rounded-xl">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-textSecondary mb-4">
                If there's a disagreement about the work or payment, you can raise a dispute. 
                The platform owner will review and resolve the issue fairly.
              </p>
              <button
        ...
        <div className="p-6 bg-error/10 border border-error/20 rounded-xl">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-textSecondary">This escrow is under dispute resolution. Please wait for the platform owner to resolve.</p>
            </div>
          </div>
        </div>
      )}
